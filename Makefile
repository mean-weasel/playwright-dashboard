...
smoke-playwright-cli-dashboard-actions:
	@if [ "$$RUN_PLAYWRIGHT_CLI_DASHBOARD_ACTIONS_SMOKE" != "1" ] && [ "$$RUN_ALL_SMOKES" != "1" ]; then \
		echo "Set RUN_PLAYWRIGHT_CLI_DASHBOARD_ACTIONS_SMOKE=1 (or RUN_ALL_SMOKES=1) to run the real Playwright CLI dashboard-actions smoke test"; \
		exit 2; \
	fi
	$(MAKE) check-accessibility
	$(MAKE) validate-smoke-package
	scripts/run_smoke_with_retry.mjs --name playwright-cli-dashboard-actions -- scripts/smoke_playwright_cli_dashboard_actions.mjs

smoke-playwright-cli-multi-target:
	@if [ "$$RUN_PLAYWRIGHT_CLI_MULTI_TARGET_SMOKE" != "1" ] && [ "$$RUN_ALL_SMOKES" != "1" ]; then \
		echo "Set RUN_PLAYWRIGHT_CLI_MULTI_TARGET_SMOKE=1 (or RUN_ALL_SMOKES=1) to run the real Playwright CLI multi-target smoke test"; \
		exit 2; \
	fi
	$(MAKE) check-accessibility
	$(MAKE) validate-smoke-package
	scripts/run_smoke_with_retry.mjs --name playwright-cli-multi-target -- scripts/smoke_playwright_cli_multi_target.mjs

smoke-playwright-cli-recording:
	@if [ "$$RUN_PLAYWRIGHT_CLI_RECORDING_SMOKE" != "1" ] && [ "$$RUN_ALL_SMOKES" != "1" ]; then \
		echo "Set RUN_PLAYWRIGHT_CLI_RECORDING_SMOKE=1 (or RUN_ALL_SMOKES=1) to run the real Playwright CLI recording smoke test"; \
		exit 2; \
	fi
	$(MAKE) validate-smoke-package
	scripts/run_smoke_with_retry.mjs --name playwright-cli-recording -- scripts/smoke_playwright_cli_recording.mjs

smoke-playwright-cli-interaction:
	@if [ "$$RUN_PLAYWRIGHT_CLI_INTERACTION_SMOKE" != "1" ] && [ "$$RUN_ALL_SMOKES" != "1" ]; then \
		echo "Set RUN_PLAYWRIGHT_CLI_INTERACTION_SMOKE=1 (or RUN_ALL_SMOKES=1) to run the real Playwright CLI interaction smoke test"; \
		exit 2; \
	fi
	$(MAKE) check-accessibility
	$(MAKE) validate-smoke-package
...
smoke-realistic-e2e:
	@if [ "$$RUN_REALISTIC_E2E_SMOKE" != "1" ] && [ "$$RUN_ALL_SMOKES" != "1" ] && [ "$$REALISTIC_E2E_DRY_RUN" != "1" ]; then \
		echo "Set RUN_REALISTIC_E2E_SMOKE=1 (or REALISTIC_E2E_DRY_RUN=1) to run the realistic fixture E2E/demo smoke"; \
		exit 2; \
	fi
	@if [ "$$REALISTIC_E2E_DRY_RUN" = "1" ]; then \
		scripts/run_smoke_with_retry.mjs --name realistic-e2e -- scripts/smoke_playwright_cli_realistic_demo.mjs --dry-run; \
	else \
		$(MAKE) check-accessibility; \
		$(MAKE) validate-smoke-package; \
		scripts/run_smoke_with_retry.mjs --name realistic-e2e -- scripts/smoke_playwright_cli_realistic_demo.mjs; \
	fi

demo-media:
	scripts/capture_realistic_demo_media.mjs
...
smoke-all:
	...
	if [ "$$SMOKE_ALL_DRY_RUN" = "1" ]; then \
		...
		echo "  7. make smoke-playwright-cli-dashboard-actions (SKIP_ACCESSIBILITY_CHECK=1 SMOKE_REUSE_PACKAGE=1 RUN_ALL_SMOKES=1)"; \
	...
	$(MAKE) SKIP_ACCESSIBILITY_CHECK=1 SMOKE_REUSE_PACKAGE=1 RUN_ALL_SMOKES=1 smoke-playwright-cli-dashboard-actions; \
