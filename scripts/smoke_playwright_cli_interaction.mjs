...
const sessionName = `cli-interaction-${process.pid}`;
const displayName = "CLI Interaction";
await runGuiPreflight();
const tmpRoot = await fsTempDir("playwright-dashboard-cli-interaction-smoke-");
...
try {
  ...
  await openPlaywrightSession(sessionName, pageURL);
  const debugPort = await loadDebugPortFromSessionFile(sessionName);

  await waitFor(async () => {
    const pages = await json(`http://127.0.0.1:${debugPort}/json/list`).catch(() => []);
    return pages.some((page) => page.url === pageURL);
  }, "CLI-spawned Chrome CDP page");
...
async function openPlaywrightSession(id, url) {
  await runPlaywrightCLI(
    [`-s=${id}`, "open", url, "--browser=chrome", "--headed"],
    { cwd: workspaceDir, timeout: 120_000 },
  );
}

async function closePlaywrightSession(id) {
  await runPlaywrightCLI([`-s=${id}`, "close"], {
    cwd: workspaceDir,
    timeout: 10_000,
  }).catch((error) => {
    return run("pkill", ["-f", `cli-daemon/program.js ${id}`]).catch(() => {
      console.warn(`Warning: failed to close ${id}: ${error.message}`);
    });
  });
}

async function loadDebugPortFromSessionFile(id) {
  const sessionFile = await findSessionFile(daemonRoot, `${id}.session`);
  if (!sessionFile) {
    throw new Error(`playwright-cli did not create ${id}.session under ${daemonRoot}`);
  }
  ...
  if (!Number.isInteger(cdpPort) || cdpPort <= 0) {
    throw new Error(`${id}.session does not contain a usable CDP port`);
  }
  return cdpPort;
}
